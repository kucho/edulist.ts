...
function readList(filepath: string, rutaList: Ruta[], naveList: Nave[]) {
    ...
    const ruta: string = worksheet[posRuta].v.cleanSpaces();
    const trayecto: string = worksheet[posTrayecto].v.cleanSpaces();
    ...
    if (isRutaValid === false || isRutaValid === -1) { return false; }
    ...
    for (let i = 2; i < nRows; i++) {
        ...
        const nombres = worksheet[XLSX.utils.encode_cell({ r: i, c: 1 })].v.cleanSpaces();
        const apellidos = worksheet[XLSX.utils.encode_cell({ r: i, c: 2 })].v.cleanSpaces();
...
function checkConfig(
    ...
    config: { ruta: string, nave: string, fecha: string, trayecto: string, placa: string }): any {
